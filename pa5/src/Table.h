...
private:
  ...
  unsigned int hashCode(const string &word)  {
    ...
    // WARNING: However, the line will have to appear as below for it to compile 
    //     on aludra
...
// add private data and methods here
  ...
  int numNonEmptyBuckets() const;      // number of non empty buckets in the table
  int longestChain() const;     // gives the longest chain in the table
};
